...
class Cell:
    ...
    def __add__(self, other):  # сложение(add())
        return f'Сумма клеток = ({self.quantity + other.quantity})'

    def __sub__(self, other):  # вычитание(sub())
        if self.quantity - other.quantity < 0:
            return 'Разность отрицательна, поэтому операция не выполняется'
        return f'Разница клеток = ({self.quantity - other.quantity})'

    def __mul__(self, other):  # умножение(mul())
        return f'Произведение клеток  = ({self.quantity * other.quantity})'

    def __truediv__(self, other):  # деление(truediv()
        return f'Деление клеток = ({self.quantity / other.quantity:.0f})'

    def make_order(self, count):
        cp_self = deepcopy(self.quantity)
        result = ''
        for _ in range(ceil(self.quantity / count)):
            if cp_self >= count:
                result += '*' * count + r'\n '
            elif cp_self != 0:
                result += '*' * cp_self + r'\n '
