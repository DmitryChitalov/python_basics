...
class Cell:
    ...
    def __sub__(self, other):
        if (self.quantity - other.quantity) > 0:
            return f"Разность клеток = " \
                   f"{Cell(int(self.quantity - other.quantity))}"
        return f"Разность меньше нуля, поэтому операция не выполняется"

    def __mul__(self, other):
        return f"Умножение клеток = " \
               f"{Cell(int(self.quantity * other.quantity))}"

    def __truediv__(self, other):
        return f"Деление клеток = " \
               f"{Cell(round(self.quantity // other.quantity))}"
