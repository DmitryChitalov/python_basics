...
class Cell:
    ...
    def __add__(self, other_cell):
        if isinstance(other_cell, Cell):
            ...
            return 'Для сложения обе сущности должны быть клетками'
    ...
    def __sub__(self, other_cell):
        if isinstance(other_cell, Cell):
            if self.quantity - other_cell.quantity > 0:
                ...
                return 'Нельзя вычесть, разница отрицательная'
        else:
            return 'Для вычитания обе сущности должны быть клетками'

    def __mul__(self, other_cell):
        if isinstance(other_cell, Cell):
            return Cell(self.quantity * other_cell.quantity)
        else:
            return 'Для умножения обе сущности должны быть клетками'

    def __truediv__(self, other_cell):
        if isinstance(other_cell, Cell):
            return Cell(self.quantity // other_cell.quantity)
        else:
            return 'Для делеения обе сущности должны быть клетками'

    def __str__(self):
        return str(self.quantity)

    def make_order(self, line_len):
        '''Разделить количество ячеек в строки определенной длинны'''
        quantity = self.quantity
        result_string = ''
        while True:
            if quantity - line_len > 0:
                line = "*" * line_len
                quantity -= line_len
                result_string = result_string + line + '\n'
            else:
                last_iterator = line_len - quantity
                line = "*" * last_iterator
                result_string = result_string + line + '\n'
                break
