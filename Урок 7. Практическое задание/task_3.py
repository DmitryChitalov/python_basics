...
Сложение.
Объединение двух клеток. При этом число ячеек общей клетки должно
...
class Cell:
    ...
    def __add__(self, other):
        print(f"Складываем (сумма клеток): {self.quantity + other.quantity}\n")

    def __sub__(self, other):
        if (self.quantity - other.quantity) > 0:
            print(f"Вычитаем (разность клеток): {self.quantity - other.quantity}\n")
        else:
            print("Ошибка!!\nКол-во клеток меньше нуля\n")

    def __mul__(self, other):
        print(f"Умножаем (произведение клеток): {self.quantity * other.quantity}\n")

    def __truediv__(self, other):
        if other.quantity != 0:
            print(f"Делим (деление клеток): {self.quantity // other.quantity}\n")
        else:
