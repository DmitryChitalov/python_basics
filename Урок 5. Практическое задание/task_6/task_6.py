...
каждого предмета не обязательно были все типы занятий. Сформировать словарь, содержащий название
...
with open('task_6.txt', 'r', encoding='utf-8') as my_file:
    for line in my_file:
        ...
        result = sum(map(int, ''.join([i for i in time if i == ' ' or ('0' <= i <= '9')]).split()))
        discipline[name] = result
    print(f'Общее количество часов по предмету -  {discipline}')
