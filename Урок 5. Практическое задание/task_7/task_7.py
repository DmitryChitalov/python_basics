...
for i in range(0, len(content) - 1, 4):
    firm_dict[content[i]] = int(content[i + 2]) - int(content[i + 3])

average_dict['average_profit'] = sum(firm_dict.values()) / float(len(firm_dict.values()))

with open('result.json', 'w') as f_n:
    f_n.write(json.dumps([firm_dict, average_dict]))
