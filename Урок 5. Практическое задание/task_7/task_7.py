...
prof_aver = 0
i = 0
with open('task_7_text.txt', 'r', encoding='utf-8') as file:
    for line in file:
    ...
    if i != 0:
        prof_aver = prof / i
        print(f'Average profit is {prof_aver}')
    else:
        print('No profit')
    print(f'Profit of every company is {profit}')

with open('task_7_firms.json', 'w') as write_js:
    json.dump(profit, write_js)
    js_str = json.dumps(profit)
    print(f'Json file contains {js_str}')
