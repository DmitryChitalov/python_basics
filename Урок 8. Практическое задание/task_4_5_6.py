...
class Storage:
    ...
    def add(self, equipments):
        if not all([isinstance(equipment, OfficeEquipment)
    ...
    def filter_by(self, **kwargs):
        for id_, item in enumerate(self):
            if all([item.__getattribute__(key) == kwargs[key]
                   for key in kwargs]):
                yield id_, item
...
if __name__ == '__main__':
    ...
    for idx, itm in storage.filter_by(
            department=None, vendor="OKI", model="5367-AD"):
        print(f"Резервируем {itm} в 'Отдел БУ'")
        storage.transfer(idx, 'Отдел БУ')
