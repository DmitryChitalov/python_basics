...
class Warehouse:
    ...
    def reception(self):
        try:
            ...
            price = int(input(f'Введите цену за ед. в руб.: '))
            qty = int(input(f'Введите количество товара в шт.: '))
            unique = {'Модель устройства': unit,
                      'Цена за ед. в руб.': price,
                      'Количество в шт.': qty}
        ...
        print(f'Для выхода ввести *, для продолжения нажать Enter (ввод)')
        q = input()
        if q == '*':
...
unit_1 = Printer('hp', 11300, 7, 1)
unit_2 = Scanner('Samsung', 16660, 2, 2)
unit_3 = Copier('Xerox', 14900, 1, 3)
